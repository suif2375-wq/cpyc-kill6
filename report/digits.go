...
func renderDigitPane(title, key, desc string, res *position.Result) string {
	...
	historyID := key + "-recommend-history"
	b.WriteString(`<section class="digit-section digit-recommend-section"><div class="digit-section-head"><h2>10组推荐号码 <button type="button" class="digit-history-toggle" data-digit-history-toggle="` + historyID + `" aria-controls="` + historyID + `" aria-expanded="false">历史查询</button></h2><span>杀号过滤 · 位置评分 · 和值/跨度结构排序</span></div><div class="digit-recommend-grid">`)
	...
	b.WriteString(`</div><p class="digit-disclaimer">推荐组合用于覆盖不同的高分结构，不代表确定性结果；每组均先排除当前各个位杀号数字。</p>`)
	b.WriteString(renderRecommendationHistory(historyID, res.RecommendationHistory, res.Positions))
...
func renderRecommendationHistory(id string, history []position.RecommendationSnapshot, positions int) string {
	var b strings.Builder
	b.WriteString(`<div class="digit-history-panel" id="` + html.EscapeString(id) + `" hidden><div class="digit-history-toolbar"><label for="` + html.EscapeString(id) + `-search">按期号查询（最近100期）</label><input id="` + html.EscapeString(id) + `-search" class="digit-history-search" type="search" inputmode="numeric" placeholder="输入期号，如 2026237" data-digit-history-search="` + html.EscapeString(id) + `"><span class="digit-history-count" data-digit-history-count="` + html.EscapeString(id) + `"></span></div><div class="digit-history-list" data-digit-history-list="` + html.EscapeString(id) + `">`)
	if len(history) == 0 {
		...
		for _, snapshot := range history {
			...
			if len(snapshot.Recommendations) == 0 {
				b.WriteString(`<span class="digit-muted">该期数据不足，未生成推荐。</span>`)
			}
			b.WriteString(`</div></article>`)
		}
	}
	b.WriteString(`</div><div class="digit-history-empty" data-digit-history-empty="` + html.EscapeString(id) + `" hidden>未找到匹配期号。</div><p class="digit-disclaimer">历史推荐严格按开奖前可用数据生成；“开奖”仅用于事后查询对照，不参与该期推荐计算。</p></div>`)
	return b.String()
}
...
func digitCSS() string {
	return `<style>
#tab-p3:checked~.tab-bar .tab-btn[for="tab-p3"]{color:#67E8F9!important;border-color:rgba(34,211,238,.9)!important;background:linear-gradient(180deg,rgba(23,71,97,.95),rgba(13,45,65,.95))!important;box-shadow:0 0 0 1px rgba(34,211,238,.18),0 8px 20px -12px rgba(34,211,238,.8)}
#tab-p3:checked~.tab-bar .tab-btn[for="tab-p3"] .tab-ico{color:#67E8F9!important;border-color:rgba(34,211,238,.9)!important;background:rgba(34,211,238,.14)!important}
#tab-p5:checked~.tab-bar .tab-btn[for="tab-p5"]{color:#C4B5FD!important;border-color:rgba(167,139,250,.9)!important;background:linear-gradient(180deg,rgba(66,43,112,.95),rgba(39,26,76,.95))!important;box-shadow:0 0 0 1px rgba(167,139,250,.18),0 8px 20px -12px rgba(167,139,250,.8)}
#tab-p5:checked~.tab-bar .tab-btn[for="tab-p5"] .tab-ico{color:#C4B5FD!important;border-color:rgba(167,139,250,.9)!important;background:rgba(167,139,250,.14)!important}
...
.digit-main{max-width:100%}.digit-hero{padding:34px 0 42px}.digit-kicker{display:flex;align-items:center;gap:12px;color:#64748B;font-size:12px}.digit-tag{padding:4px 10px;border-radius:999px;background:rgba(34,211,238,.12);border:1px solid rgba(34,211,238,.4);color:#67E8F9;font-weight:700}.digit-hero h1{margin-top:14px;font-size:40px}.digit-sub{margin-top:12px;color:#94A3B8;line-height:1.7}.digit-sub strong{color:#67E8F9}.digit-note{max-width:780px;margin-top:16px;padding:12px 16px;border-radius:12px;background:rgba(52,211,153,.07);border:1px solid rgba(52,211,153,.28);color:#94A3B8;font-size:12px;line-height:1.7}.digit-pred-grid{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:14px;margin-top:24px}.digit-pred-card{padding:18px 16px;border-radius:16px;background:linear-gradient(180deg,rgba(23,71,97,.75),rgba(13,20,41,1));border:1px solid rgba(34,211,238,.32);min-height:150px;display:flex;flex-direction:column;justify-content:space-between}.digit-pred-head{display:flex;justify-content:space-between;gap:8px;color:#67E8F9;font-size:12px}.digit-pred-head small{color:#94A3B8;font-size:10px}.digit-pred-num{font:800 34px/1.1 var(--font-num);text-align:center;color:#22D3EE;letter-spacing:1px}.digit-pred-foot{font-size:10px;color:#64748B;text-align:center}.digit-section{padding:0 0 34px}.digit-section-head{display:flex;justify-content:space-between;align-items:end;margin-bottom:14px}.digit-section-head h2{font-size:22px}.digit-section-head span{font-size:11px;color:#64748B}.digit-summary{display:grid;grid-template-columns:320px 1fr;gap:16px}.digit-summary-main,.digit-stat{padding:18px;border-radius:14px;background:#121A2B;border:1px solid rgba(148,163,184,.12)}.digit-summary-main{display:flex;flex-direction:column;gap:10px}.digit-summary-main span{color:#6EE7B7;font-size:13px}.digit-summary-main strong{font:800 40px var(--font-num);color:#34D399}.digit-summary-main em{font-size:11px;color:#94A3B8;font-style:normal}.digit-bar{height:6px;border-radius:3px;background:#1E293B;overflow:hidden}.digit-bar i{display:block;height:100%;border-radius:3px;background:linear-gradient(90deg,#34D399,#22D3EE)}.digit-stats{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:12px}.digit-stat{display:flex;flex-direction:column;gap:7px}.digit-stat span{font-size:12px;color:#94A3B8}.digit-stat strong{font:800 26px var(--font-num);color:#67E8F9}.digit-stat small{font-size:10px;color:#64748B}.digit-chart{padding:12px;border-radius:14px;background:#0D1424;border:1px solid rgba(148,163,184,.12);overflow:auto}.digit-chart svg{display:block;width:100%;min-width:620px;height:210px}.digit-table-wrap{overflow:auto;border-radius:14px;border:1px solid rgba(148,163,184,.12)}.digit-table{width:100%;border-collapse:collapse;min-width:680px;background:#0D1424}.digit-table th,.digit-table td{padding:10px 12px;border-bottom:1px solid rgba(148,163,184,.1);text-align:left;font-size:11px}.digit-table th{color:#94A3B8;background:#121A2B;font-weight:600}.digit-table td{color:#CBD5E1}.digit-table .digit-open{font:700 14px var(--font-num);color:#E8EEF9;letter-spacing:1px}.digit-table .digit-ok{color:#34D399;font-weight:700}.digit-table .digit-fail{color:#F87171}.digit-disclaimer,.digit-muted{margin-top:12px;color:#64748B;font-size:11px;line-height:1.6}@media(max-width:900px){.digit-pred-grid{grid-template-columns:repeat(3,minmax(0,1fr))}.digit-summary{grid-template-columns:1fr}.digit-stats{grid-template-columns:repeat(2,minmax(0,1fr))}}@media(max-width:560px){.digit-pred-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.digit-hero h1{font-size:30px}.digit-section-head{display:block}.digit-section-head span{display:block;margin-top:6px}.digit-stats{grid-template-columns:1fr}}
 .digit-recommend-section{margin-top:4px}.digit-recommend-grid{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:12px}.digit-recommend-card{position:relative;padding:16px 14px;border-radius:14px;background:linear-gradient(145deg,rgba(30,41,59,.9),rgba(15,23,42,.95));border:1px solid rgba(96,165,250,.24);min-height:138px;display:flex;flex-direction:column;justify-content:center;align-items:center;gap:7px}.digit-recommend-card:nth-child(-n+3){border-color:rgba(52,211,153,.4);box-shadow:0 10px 24px -14px rgba(52,211,153,.65)}.digit-recommend-rank{position:absolute;left:10px;top:8px;color:#64748B;font:700 11px var(--font-num)}.digit-recommend-number{font:800 27px var(--font-num);letter-spacing:2px;color:#E8EEF9}.digit-recommend-meta{font:600 11px var(--font-num);color:#67E8F9}.digit-recommend-reasons{font-size:10px;line-height:1.5;text-align:center;color:#94A3B8}.digit-recommend-card:nth-child(3n+2) .digit-recommend-number{color:#A78BFA}.digit-recommend-card:nth-child(3n+3) .digit-recommend-number{color:#FBBF24}.digit-history-toggle{margin-left:10px;padding:5px 10px;border:1px solid rgba(34,211,238,.45);border-radius:999px;background:rgba(34,211,238,.08);color:#67E8F9;font-size:11px;font-weight:600;vertical-align:middle;cursor:pointer}.digit-history-toggle:hover{background:rgba(34,211,238,.18)}.digit-history-panel{margin-top:16px;padding:16px;border-radius:14px;background:rgba(13,20,36,.82);border:1px solid rgba(148,163,184,.16)}.digit-history-panel[hidden]{display:none}.digit-history-toolbar{display:flex;align-items:center;gap:10px;flex-wrap:wrap;margin-bottom:12px}.digit-history-toolbar label{font-size:12px;color:#CBD5E1}.digit-history-search{height:34px;min-width:230px;padding:0 10px;border:1px solid rgba(148,163,184,.28);border-radius:8px;background:#0A0E17;color:#E8EEF9;outline:none}.digit-history-search:focus{border-color:#22D3EE;box-shadow:0 0 0 3px rgba(34,211,238,.12)}.digit-history-count{font-size:11px;color:#64748B}.digit-history-list{max-height:360px;overflow:auto;display:flex;flex-direction:column;gap:8px}.digit-history-item{padding:11px 12px;border-radius:10px;background:rgba(18,26,43,.72);border:1px solid rgba(148,163,184,.1)}.digit-history-item-head{display:flex;justify-content:space-between;gap:12px;align-items:center;margin-bottom:8px}.digit-history-item-head strong{font-size:12px;color:#67E8F9}.digit-history-item-head span{font-size:10px;color:#64748B}.digit-history-numbers{display:flex;gap:6px;flex-wrap:wrap}.digit-history-number{display:inline-flex;align-items:center;gap:4px;padding:5px 7px;border-radius:7px;background:rgba(34,211,238,.07);border:1px solid rgba(34,211,238,.2);color:#E8EEF9;font:700 12px var(--font-num)}.digit-history-number b{font-size:9px;color:#64748B}.digit-history-empty{padding:14px;text-align:center;color:#94A3B8;font-size:12px}.digit-history-empty[hidden]{display:none}@media(max-width:900px){.digit-recommend-grid{grid-template-columns:repeat(3,minmax(0,1fr))}}@media(max-width:560px){.digit-recommend-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.digit-recommend-number{font-size:23px}.digit-history-item-head{display:block}.digit-history-item-head span{display:block;margin-top:4px}.digit-history-search{min-width:180px;width:100%}}
</style>`
}
